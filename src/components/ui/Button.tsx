...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const baseStyles =
      "inline-flex items-center justify-center font-semibold rounded-2xl transition-all duration-300 focus:outline-none focus:ring-4 disabled:opacity-50 cursor-pointer disabled:pointer-events-none active:scale-[0.98] hover:-translate-y-1 hover:shadow-md";

    const variants = {
      primary:
        "bg-primary text-primary-background hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] focus:ring-primary/20",
      secondary:
        "bg-secondary-background text-secondary-foreground hover:bg-muted-background focus:ring-secondary-background/50 border border-transparent hover:border-black/5",
      danger:
